...
def get():
    ...
    if access_token:
        try:
            ...
            response = make_response({"data": booked_info}, 200, headers)
            return response
...
def post():
    ...
    access_token = request.cookies.get("access_token")
    request_validate = model_validate_input.validate_booking_request(booking_request)
    if not request_validate:
        response = make_response(
            {"error": True, "message": "建立失敗，輸入不正確或其他原因"}, 400, headers
        )
        return response
    if access_token:
        try:
