...
class User:
    ...
    def email_validate(self, request):
        email_validate_query = """
            SELECT EXISTS (SELECT * 
                           FROM user 
                           WHERE email = %s)
        """
        val = (request["email"],)
        result = mysql_pool.execute(email_validate_query, val)[0]
        keys = list(result.keys())
        repeat_email_num = result[keys[0]]
        if repeat_email_num == 0:
            return True
        else:
            return False
    ...
    def signup(self, request):
        signup_query = """
            ...
            VALUES (%s, %s, %s);
        """
        hashed_password = bcrypt.generate_password_hash(request["password"]).decode(
            "utf8"
        )
        val = (
            request["name"],
    ...
    def signin(self, request):
        ...
        password_check = bcrypt.check_password_hash(user_password, request["password"])
